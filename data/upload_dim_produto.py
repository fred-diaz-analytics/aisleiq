"""
Upload dos CSVs do domínio `produtos` (dado mestre, gerados por
export_dim_produto.py) pro storage S3-compatible. Entrypoint fino — a lógica
...
Uso:
  python upload_dim_produto.py
  python upload_dim_produto.py --out-dir ./data --env-file ./data/.env
"""
import argparse
import os
...
ARQUIVOS = ["categorias.csv", "produtos.csv"]
PREFIXO_REMOTO = "trade_analytics/produtos"


def upload_dim_produto(out_dir: str) -> tuple[int, int]:
    ok, falhou = 0, 0
...
if __name__ == "__main__":
    ...
    total_ok, total_falhou = upload_dim_produto(args.out_dir)
