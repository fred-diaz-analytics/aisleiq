...
# MAGIC %md
# MAGIC # Bronze — Ingestão do domínio `lojas` (dado mestre)
# MAGIC
# MAGIC Diferente de `bronze_ingest.py` (incremental, por data): `lojas` é dado
# MAGIC mestre estático, 5 CSVs fixos (`estados`, `cidades`, `categorias_loja`,
# MAGIC `redes`, `lojas`), sem partição por data, recarregados por completo a
# MAGIC cada execução (`CREATE OR REPLACE TABLE`, sem watermark/lookback/
# MAGIC `replaceWhere`). Roda sob demanda, via Job próprio
# MAGIC (`trade_analytics_lojas`), não entra no job diário agendado.

# COMMAND ----------

dbutils.widgets.text("bucket", "datalake", "Bucket S3 (Supabase)")
dbutils.widgets.text("prefix", "trade_analytics/lojas/", "Prefixo no bucket")
...
TABELAS = ["estados", "cidades", "categorias_loja", "redes", "lojas"]

logging.basicConfig(level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s")
log = logging.getLogger("bronze_ingest_lojas")
