...
# MAGIC %md
# MAGIC # Bronze — Ingestão do domínio `metas` (metas comerciais, dado mestre)
# MAGIC
# MAGIC Mesmo padrão de `bronze_ingest_produtos.py`: dado mestre estático, 3 CSVs
# MAGIC fixos (`preco_sugerido`, `sku_prioridade`, `meta_share`), sem partição
# MAGIC por data, recarregados por completo a cada execução (sem watermark/
# MAGIC lookback/`replaceWhere`). Roda sob demanda, via Job próprio
# MAGIC (`trade_analytics_metas`), não entra no job diário agendado.

# COMMAND ----------

dbutils.widgets.text("bucket", "datalake", "Bucket S3 (Supabase)")
dbutils.widgets.text("prefix", "trade_analytics/metas/", "Prefixo no bucket")
...
TABELAS = ["preco_sugerido", "sku_prioridade", "meta_share"]

logging.basicConfig(level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s")
log = logging.getLogger("bronze_ingest_metas")
